...
.github/workflows/        add-to-project, pr-assign, pr
```
...
`add-to-project.yml` puts new issues and pull requests on a project board; it stays idle until the `PROJECT_URL` variable and `ADD_TO_PROJECT_PAT` secret are set. `pr-assign.yml` assigns the author, skipping forks and bots.

## Deployments

Three Vercel projects build from this repo, all with root directory `.`, so they share the root `vercel.json` and differ only in their build command and output directory:

| Project | Build command | Output directory |
|---------|---------------|------------------|
| demo.dappbooster | `pnpm build` | (default `dist`) |
| components.dappbooster | `pnpm typedoc:build` | `typedoc` |
| docs.dappbooster | `pnpm docs:build` | ignored, see below |
...
The adapter writes it relative to the working directory, and `docs:build` runs from `docs/`, so the script moves `docs/.vercel/output` up to `.vercel/output` where Vercel looks. Locally `VERCEL` is unset, nothing is written, and `pnpm docs:preview` serves the ordinary vocs output from `docs/dist`.

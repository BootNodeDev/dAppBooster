...
This package provides a flexible and reusable solution for generating [GraphQL](https://graphql.org/) clients using `@graphql-codegen/cli`, integrated with `@tanstack/react-query`. It simplifies the process of setting up and managing GraphQL code generation for multiple subgraphs.

## Installation

The plugin uses pnpm as the preferred package manager. First, install the package along with other required dependencies:
...
Create a codegen file `src/subgraphs/codegen.ts` in your project:

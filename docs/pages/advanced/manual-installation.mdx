...
The default branch is `develop`, which is fine if you're developing new features of fixing bugs, but if you're developing a new product you probably want the production-ready release.

```bash [Terminal]
git checkout main
```

Or you can also get the last release (usually in sync with `main`), which is what the install script does.

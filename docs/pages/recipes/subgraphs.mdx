...
This guide will walk you through the use of the [subgraphs plugin](https://www.npmjs.com/package/@bootnodedev/db-subgraph) in dAppBooster, before continuing, be sure to have followed the steps in the [Subgraphs Plugin](/advanced/subgraph-plugin) section.
...
::::steps
### Create a query to list Uniswap pools by liquidity

Create a new file `src/subgraphs/queries/uniswap/pool.ts` with the following content:
...
### Create a component to consume the data

Create a new component in `src/components/sharedComponents/UniswapPoolsPolygon.tsx` that will consume and show the pool data.
...
:::tip
You can find a working example of this in the [demos list](https://github.com/BootNodeDev/dAppBooster/blob/main/src/components/pageComponents/home/Examples/demos/subgraphs/Subgraph/index.tsx)
:::